...
function useAuth() {
  ...
  const register = useCallback(
    async (values: User, successCallback?: Function) => {
      setLoading(true);
      axios
        .post(`${apiBasePath}/register`, values)
        .then((res) => {
          setUser(res.data);
  ...
  const login = useCallback(
    async (values: UserLogin, successCallback?: Function) => {
      setLoading(true);
      const formData = new FormData();
      formData.append('username', values.username);
      formData.append('password', values.password);
      axios
        .post(`${apiBasePath}/token`, formData)
        .then((res) => {
          setLoading(false);
