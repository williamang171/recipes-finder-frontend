...
function useRecipes() {
  ...
  const createRecipe = useCallback(
    async (values: Recipe) => {
      setLoading(true);
      axios
        .post(`${apiBasePath}/`, values, getAuthHeaderOptions())
        .then((res) => {
          const newRecipes = [...recipes, res.data];
  ...
  const removeRecipe = useCallback(
    async (id) => {
      setLoading(true);
      axios
        .delete(`${apiBasePath}/${id}`, getAuthHeaderOptions())
        .then(() => {
  ...
  const getRecipes = useCallback(async () => {
    ...
    axios
      .get(`${apiBasePath}/`, getAuthHeaderOptions())
      .then((res) => {
        setLoading(false);
